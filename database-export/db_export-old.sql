...
CREATE TABLE `categoriebillet` (
  `IdCategorie` int(11) NOT NULL,
  `IntituleCategorie` varchar(250) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `codepromo` (
  `CodePromo` varchar(250) NOT NULL,
  `NbrUtilisation` int(11) NOT NULL,
...
CREATE TABLE `place` (
  ...
  `CategoriePlace` varchar(250) NOT NULL,
  `IdBillet` int(11) NOT NULL,
  `IdMatch` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
--
-- Structure de la table `typetournois`
--

CREATE TABLE `typetournois` (
  `IdTypeTournois` int(11) NOT NULL,
  `IntituleType` varchar(250) NOT NULL,
  `IdBillet` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
--
ALTER TABLE `place`
  ADD KEY `Billet` (`IdBillet`),
  ADD KEY `Match` (`IdMatch`);
...
--
-- Index pour la table `typetournois`
--
ALTER TABLE `typetournois`
  ADD PRIMARY KEY (`IdTypeTournois`),
  ADD KEY `fk_IdBillet` (`IdBillet`);
...
--
-- AUTO_INCREMENT pour la table `categoriebillet`
--
ALTER TABLE `categoriebillet`
...
--
-- AUTO_INCREMENT pour la table `typetournois`
--
ALTER TABLE `typetournois`
  MODIFY `IdTypeTournois` int(11) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `achat`
  ADD CONSTRAINT `IdBillet` FOREIGN KEY (`IdBillet`) REFERENCES `billet` (`IdBillet`) ON DELETE CASCADE ON UPDATE CASCADE,
...
--
ALTER TABLE `place`
  ADD CONSTRAINT `Billet` FOREIGN KEY (`IdBillet`) REFERENCES `billet` (`IdBillet`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `Match` FOREIGN KEY (`IdMatch`) REFERENCES `matchs` (`idMatch`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Contraintes pour la table `typetournois`
--
ALTER TABLE `typetournois`
  ADD CONSTRAINT `fk_IdBillet` FOREIGN KEY (`IdBillet`) REFERENCES `billet` (`IdBillet`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
